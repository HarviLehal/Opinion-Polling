...
plot2<-ggplot(data=d2, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
...
scale_fill_manual(values = c("#ed8e98","#D82C3C","#d68bba","#AC347D",
                             ...
                             "#81a6d4","#2760A7"))+
geom_text(aes(label = formattable::percent(ifelse(d2$Date != min(d2$Date), d2$value, "")),
              y = 0),
          hjust=0, color="#000000",position = position_dodge(1), size=3.5)+
geom_text(aes(label = ifelse(d2$Date == min(d2$Date),paste("(",d2$value,")"),""),
              y = 0),
