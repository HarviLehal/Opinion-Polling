...
plot<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  geom_point(size=1, data=d[d$Date!=election,],alpha=0.75)+
  scale_color_manual(values = c("#ffb300","#ed1c24",
                                "#3e4042","#0d5ca6"))+
  geom_smooth(method='loess',fullrange=TRUE,span=0.5,se=FALSE,linewidth=0.75, data=d[d$Date!=election,])+
