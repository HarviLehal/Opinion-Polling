...
py_run_file("Turkish/President/data2.py")
poll <- read_csv("Turkish/President/poll2.csv")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_point(size=1, data=d[d$Date!=election,],alpha=0.25)+
  scale_color_manual(values = c("#ed1c24","#FFCC00"))+
  ...
  ggtitle('Second Round Polling for the 2028 Turkish Presidential Election')
plot1
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#ed1c24",
                               "#FFCC00"))+
...
ggsave(plot=plot, file="Turkish/President/plot2.png",width = 15, height = 7.5, type = "cairo-png")
ggsave(plot=plot, file="Turkish/President/plot2.svg",width = 15, height = 7.5)
aaa=readLines("Turkish/President/plot2.svg",-1)
bbb <- gsub(".svglite ", "", aaa)
writeLines(bbb,"Turkish/President/plot2.svg")
