...
library(data.table)
library(hrbrthemes)
py_run_file("Australia_Elections/data.py")
poll <- read_csv("Australia_Elections/poll.csv")
...
ggsave(plot=plot, file="Australia_Elections/plot.png",width = 15, height = 7.5, type="cairo-png")


# COALITION

poll <- read_csv("Australia_Elections/poll2.csv")
d <- melt(poll, id.vars="Date")
d$value<-as.numeric(sub("%","",d$value))/100
...
ggsave(plot=plot2, file="Australia_Elections/plot2.png",width = 15, height = 7.5, type="cairo-png")
