...
library(data.table)
library(hrbrthemes)
py_run_file("Spain/Region/Catalonia/data.py")
poll <- read_csv("Spain/Region/Catalonia/poll.csv")
...
old <-min(d$Date)
# MAIN GRAPH
parties<-d[d$variable!='Podemos',]
podemos<-d[d$variable=='Podemos',]
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_color_manual(values = c("#ef1c27","#00c7ae","#ffb232",
                                "#1d84ce","#63be21","#ad275c",
                                "#ffde4b","#054a81","#9369f5"))+
  ...
  ggtitle('Opinion Polling for the Next Catalonian Regional Election')
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  geom_text(aes(label = ifelse(d4$Date == min(d4$Date),ifelse(is.na(d4$value)==FALSE,
                               paste("(",formattable::percent(d4$value, digits = 2, decimal.mark = ","),")"),paste("(Part of Comuns)")),""),
  ...
  ggtitle('2 Week Average <br> *(2023 Result)*')+
...
ggsave(plot=plot, file="Spain/Region/Catalonia/plot.png",width = 15, height = 7.5, type="cairo-png")
