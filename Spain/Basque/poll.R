...
library(data.table)
library(hrbrthemes)
py_run_file("Spain/Basque/data.py")
poll <- read_csv("Spain/Basque/poll.csv")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_color_manual(values = c("#48af49","#00ae8f","#ef1c27",
                                "#9369f5","#63be21","#1d84ce","#ef4b91"))+
  geom_smooth(method="loess",fullrange=TRUE,se=FALSE,span=0.7,linewidth=0.75, data=d[d$Date!=old&d$Date!=election,])+
  # bbplot::bbc_style()+
...
ggsave(plot=plot, file="Spain/Basque/plot.png",width = 15, height = 7.5, type="cairo-png")
