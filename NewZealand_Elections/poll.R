...
library(data.table)
library(hrbrthemes)
py_run_file("NewZealand_Elections/data.py")
poll <- read_csv("NewZealand_Elections/poll.csv")
...
ggsave(plot=plot, file="NewZealand_Elections/plot.png",width = 15, height = 7.5, type="cairo-png")


# COALITION

poll <- read_csv("NewZealand_Elections/poll2.csv")
d <- melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)
h <- formattable::percent(0.05)
...
ggsave(plot=plot2, file="NewZealand_Elections/plot2.png",width = 15, height = 7.5, type="cairo-png")
