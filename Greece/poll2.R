...
election<-as.Date("25 06 2023", "%d %m %Y")
old <-min(d$Date)
# MAIN GRAPH
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#325BC7","#8fa5e3","#325BC7",
...
ggsave(plot=plot, file="Greece/plot.svg",width = 15, height = 7.5)
aaa=readLines("Greece/plot.svg",-1)
bbb <- gsub(".svglite ", "", aaa)
writeLines(bbb,"Greece/plot.svg")
