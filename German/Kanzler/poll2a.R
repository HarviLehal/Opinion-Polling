...
library(data.table)
library(hrbrthemes)
poll <- read_csv("German/Kanzler/poll2a.csv")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable))+
  ...
  scale_color_manual(values = c("#DD1529","#005974","#509A3A","#aaaaaa"))+
  ...
  scale_y_continuous(name="Vote",labels = scales::percent_format(accuracy = 5L),breaks=seq(0,0.99,0.05))+
  scale_x_date(date_breaks = "2 month", date_labels =  "%b %Y",limits = c(old,election),guide = guide_axis(angle = -90))+
  ggtitle('Preferred German Chancellor Polling (Excluding Neither)')
...
plot2<-ggplot(data=d1, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#DD1529","#005974","#509A3A","#aaaaaa"))+
...
ggsave(plot=plot, file="German/Kanzler/plot2a.png",width = 15, height = 7.5, type="cairo-png")
