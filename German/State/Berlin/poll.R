...
library(data.table)
library(hrbrthemes)
py_run_file("German/State/Berlin/data.py")
poll <- read_csv("German/State/Berlin/poll.csv")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)
h <- formattable::percent(0.05)

election<-as.Date("18 09 2026", "%d %m %Y")
old <-min(d$Date)
# LOESS GRAPH

plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  geom_point(size=1, data=d[d$Date!=old&d$Date!=election,],alpha=0.5)+
  scale_color_manual(values = c("#005974","#DD1529","#509A3A","#B43377",
                                "#009EE0","#FBBE00","#792350","#aaaaaa"))+
  ...
  ggtitle('Opinion Polling for the Next Berlin State Election')
plot1
...
#                              "#fbe397","#fdd866","#FBBE00"))+
  ...
  geom_text(aes(label = ifelse(d4$Date != min(d4$Date),
                               ifelse(d4$Date == max(d4$Date),ifelse(is.na(d4$value)==F,
  ...
  geom_text(aes(label = ifelse(d4$Date == min(d4$Date),ifelse(is.na(d4$value)==FALSE,paste("(",formattable::percent(d4$value, digits = 1),")"),paste("(New)")),""),
  ...
  theme(legend.position = "none",axis.title=element_blank(),axis.text.x = element_blank(),
        ...
        panel.background = element_rect(fill="#FFFFFF",color="#FFFFFF"),
        plot.background = element_rect(fill = "#FFFFFF",color="#FFFFFF"))+
  ggtitle(' Latest Poll <br> *(2023 Election)*')+
...
ggsave(plot=plot, file="German/State/Berlin/plot.png",width = 15, height = 7.5, type="cairo-png")
ggsave(plot=plot, file="German/State/Berlin/plot.svg",width = 15, height = 7.5)
aaa=readLines("German/State/Berlin/plot.svg",-1)
bbb <- gsub(".svglite ", "", aaa)
writeLines(bbb,"German/State/Berlin/plot.svg")
