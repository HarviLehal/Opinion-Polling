...
library(data.table)
library(hrbrthemes)
py_run_file("German/State/Saxony-Anhalt/data.py")
poll <- read_csv("German/State/Saxony-Anhalt/poll.csv")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)

election<-as.Date("1 6 2026", "%d %m %Y")
old <-min(d$Date)
# MAIN GRAPH
parties<-d[d$variable!='BSW'|d$variable!='V',]
BSW<-d[d$variable=='BSW'&d$variable=='Volt',]
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_color_manual(values = c("#005974","#009EE0","#B43377",
                                "#DD1529","#FBBE00","#509A3A",
                                "#792350","#aaaaaa"))+
  ...
  ggtitle('Opinion Polling for the 2026 Saxony-Anhalt State Election')
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  ggtitle('Latest Poll <br> *(2021 Result)*')+
...
ggsave(plot=plot, file="German/State/Saxony-Anhalt/plot.png",width = 15, height = 7.5, type="cairo-png")
ggsave(plot=plot, file="German/State/Saxony-Anhalt/plot.svg",width = 15, height = 7.5)
aaa=readLines("German/State/Saxony-Anhalt/plot.svg",-1)
bbb <- gsub(".svglite ", "", aaa)
writeLines(bbb,"German/State/Saxony-Anhalt/plot.svg")
