...
wikiurl="https://en.wikipedia.org/wiki/Opinion_polling_for_the_2024_South_Korean_legislative_election"
...
for i in range(2):
  ...
  if i ==0:
    ...
    headers = ['Date','DAK','PFP','GJP','Other']
    parties = ['DAK','PFP','GJP','Other']
    d[i]=d[i].drop(['Polling firm','Sample size','Margin of error','LUP','Und./ no ans.','Lead'], axis=1)
  d[i].columns = headers
  d[i]['Date2'] = d[i]['Date'].str.split('–').str[1]
  d[i].Date2.fillna(d[i]['Date'].str.split('-').str[1], inplace=True)
