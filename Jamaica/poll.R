...
library(data.table)
library(hrbrthemes)
py_run_file("Jamaica/data.py")
poll <- read_csv("Jamaica/poll.csv")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)

election<-as.Date("01 11 2025", "%d %m %Y")
old <-min(d$Date)
# MAIN GRAPH
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_color_manual(values = c("#00a94c","#fb740a"))+
  ...
  geom_hline(aes(yintercept=1), alpha=0)+
  xlim(min(d$Date), election)+
  geom_vline(xintercept=old, linetype="solid", color = "#56595c", alpha=0.5, size=0.75)+
  geom_point(data=d[d$Date==old|d$Date==election,],size=5, shape=18, alpha=0.5)+
  geom_point(data=d[d$Date==old|d$Date==election,],size=5.25, shape=5, alpha=0.5)+
  scale_x_date(date_breaks = "2 month", date_labels =  "%b %Y",limits = c(old,election),guide = guide_axis(angle = -90))+
  ggtitle('Opinion Polling for the 2025 Jamaican general election')
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#66cb94","#00a94c",
                               "#fdac6c","#fb740a"))+
...
ggsave(plot=plot, file="Jamaica/plot.png",width = 15, height = 7.5, type="cairo-png")
