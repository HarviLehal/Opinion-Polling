...
library(data.table)
library(hrbrthemes)
py_run_file("Canada/Provincial/Quebec/federales_data.py")
poll <- read_csv("Canada/Provincial/Quebec/federales_poll.csv")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  ggtitle('Sondages sur les élections fédérales canadiennes de 2025 au Québec')
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  # ggtitle('5 Day Average <br> *(2021 Result)*')+
