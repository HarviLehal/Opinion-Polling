...
poll <- read_csv("Canada/Provincial/Ontario/poll2.csv")
...
start <- as.Date("29 01 2025", "%d %m %Y")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_x_break(c(old+1, start+1))+
  scale_x_date(date_breaks = "2 days", date_labels =  "%d %b %Y",limits = c(old-1,election),guide = guide_axis(angle = -90))+
  ggtitle('Sondage sur la période de campagne électorale pour les élections générales ontariennes de 2025')
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  theme(legend.position = "none",
        ...
        panel.background = element_rect(fill="#FFFFFF",color="#FFFFFF"),
        plot.background = element_rect(fill = "#FFFFFF",color="#FFFFFF"))+
  # ggtitle(' Résultats 2024 <br> Moyenne sur 5 jours <br> *(Résultats 2022)*')+
  ggtitle(' Résultats 2025 <br> Moyenne sur 5 jours <br> *(Résultats 2022)*')+
...
ggsave(plot=plot, file="Canada/Provincial/Ontario/plot2.png",width = 15, height = 7.5, type="cairo-png")
ggsave(plot=plot, file="Canada/Provincial/Ontario/plot2.svg",width = 15, height = 7.5)
aaa=readLines("Canada/Provincial/Ontario/plot2.svg",-1)
bbb <- gsub(".svglite ", "", aaa)
writeLines(bbb,"Canada/Provincial/Ontario/plot2.svg")
