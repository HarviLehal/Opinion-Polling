...
poll <- read_csv("Canada/Provincial/British Columbia/poll2.csv")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)

election<-as.Date("19 Oct 2024", "%d %b %Y")
start <- as.Date("22 Sep 2024", "%d %b %Y")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_color_manual(values = c("#EF7B00",
                                "#19bfd2",
                                "#3D9F3B","#00529F"))+
  ...
  ggtitle('Campaign Period Polling for the 2024 British Columbia general election')
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#f9ca99","#f5b066","#EF7B00",
                               "#a3e5ed","#75d9e4","#19bfd2",
                               "#b1d9b1","#8bc589","#3D9F3B","#9ab4f2","#6F94ED","#00529F"))+
  ...
  theme(legend.position = "none",axis.title=element_blank(),axis.text.x = element_blank(),
        ...
        panel.background = element_rect(fill="#FFFFFF",color="#FFFFFF"),
        plot.background = element_rect(fill = "#FFFFFF",color="#FFFFFF"))+
  ggtitle('Results <br> 5 day Average <br> *(2020 Election)*')+
...
ggsave(plot=plot, file="Canada/Provincial/British Columbia/plot2.png",width = 15, height = 7.5, type="cairo-png")