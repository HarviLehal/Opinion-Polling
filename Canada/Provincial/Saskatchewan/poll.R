...
library(data.table)
library(hrbrthemes)
py_run_file("Canada/Provincial/Saskatchewan/data.py")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_smooth(method="loess",fullrange=FALSE,se=FALSE,span=0.55,linewidth=0.75, data=d[d$Date!=old&d$Date!=election,])+
  theme_minimal()+
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#99bcb2","#669b8c","#00583f","#ffd699","#ffc266","#ff9900",
                               "#feeb9b","#fee068","#fdcc04","#99d3bb","#66bd99","#009155",
                               "#d6d6ff","#c2c2ff","#9999ff","#f7c5c3","#f2a7a6","#ea6d6a",
                               "#99becb","#669eb1","#005d7d"))+
  ...
  theme(legend.position = "none",
        ...
        panel.background = element_rect(fill="#FFFFFF",color="#FFFFFF"),
        plot.background = element_rect(fill = "#FFFFFF",color="#FFFFFF"))+
  ggtitle(' 2024 Results <br> 7 Day Average <br> *(2020 Results)*')+
...
ggsave(plot=plot, file="Canada/Provincial/Saskatchewan/plot.png",width = 15, height = 7.5, type="cairo-png")