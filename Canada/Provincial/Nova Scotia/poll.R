...
library(data.table)
library(hrbrthemes)
py_run_file("Canada/Provincial/Nova Scotia/data.py")
poll <- read_csv("Canada/Provincial/Nova Scotia/poll.csv")
Sys.setlocale("LC_ALL", "French")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)

election<-as.Date("26 11 2024", "%d %m %Y")
old <-min(d$Date)
# MAIN GRAPH
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_color_manual(values = c("#9999ff","#ea6d6a","#f4a460",
                                "#99c955","#aaaaaa"))+
  ...
  ggtitle('Sondages sur les élections générales néo-écossaises de 2024')
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  ggtitle(' Résultats 2024 <br> Moyenne sur la semaine <br> *(Résultats 2021)*')+
...
ggsave(plot=plot, file="Canada/Provincial/Nova Scotia/plot.png",width = 15, height = 7.5, type="cairo-png")
