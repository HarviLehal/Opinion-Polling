...
library(data.table)
library(hrbrthemes)
py_run_file("Canada/Provincial/New Brunswick/data.py")
poll <- read_csv("Canada/Provincial/New Brunswick/poll.csv")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)

election<-as.Date("21 10 2024", "%d %m %Y")
old <-min(d$Date)
# MAIN GRAPH
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_color_manual(values = c("#9999ff","#ea6d6a",
                                "#99c955","#aa99cc","#f4a460"))+
...
ggsave(plot=plot, file="Canada/Provincial/New Brunswick/plot.png",width = 15, height = 7.5, type="cairo-png")
