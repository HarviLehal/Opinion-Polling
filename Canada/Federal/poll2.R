...
poll <- read_csv("Canada/Federal/poll2.csv")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)

election<-as.Date("28 Apr 2025", "%d %b %Y")
start <- as.Date("23 Mar 2025", "%d %b %Y")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_color_manual(values = c("#00529F","#D91920",
                                "#EF7B00","#127C73",
                                "#442D7B","#3D9F3B"))+
  geom_smooth(method="loess",fullrange=FALSE,se=FALSE,span=0.08,linewidth=0.75, data=d[d$Date!=old&d$Date!=election,])+
  theme_minimal()+
  ...
  ggtitle('Campaign Period Polling for the 2025 Canadian Federal Election')
...
ggsave(plot=plot, file="Canada/Federal/plot2.png",width = 15, height = 7.5, type="cairo-png")