...
py_run_file("Finnish_Elections/data.py")
poll <- read_csv("Finnish_Elections/poll.csv")
...
ggsave(plot=plot, file="Finnish_Elections/plot.png",width = 15, height = 7.5,type="cairo-png")



poll <- read_csv("Finnish_Elections/poll2.csv")
d <- melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)
...
ggsave(plot=plot, file="Finnish_Elections/plot2.png",width = 15, height = 7.5,type="cairo-png")