...
scale_fill_manual(values = c("#F7B7B7","#e0837e","#CC3028",
                             "#B5CEFF","#6697c5","#00529F",
                             "#9EED8E","#6bb387","#098137",
                             "#fadc7d","#fdd866","#FBBE00",
                             "#e8b3b8","#d16671","#b30013",
                             "#666666","#8c8a8a","#403c3c"))+
...
plot1a<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  geom_point(size=1, data=d[d$Date!=old&d$Date!=election,],alpha=0.5)+
  scale_color_manual(values = c("#CC3028","#00529F","#403c3c"))+
  geom_smooth(method="loess",fullrange=TRUE,se=FALSE,span=0.55,linewidth=0.75, data=d[d$Date!=old&d$Date!=election,])+
  # bbplot::bbc_style()+
  theme(axis.title=element_blank(),legend.title = element_blank(),
        legend.key.size = unit(2, 'lines'),
