...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_point(data=d[d$Date==old,],size=5.25, shape=5, alpha=0.5)+
  scale_x_date(date_breaks = "3 month", date_labels =  "%b %Y",limits = c(old-64,election),guide = guide_axis(angle = -45))+
...
ggsave(plot=plot, file="NewZealand/plot.png",width = 15, height = 7.5, type="cairo-png")
...
plot1a<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  geom_point(size=1, data=d[d$Date!=old&d$Date!=election,],alpha=0.5)+
  scale_color_manual(values = c("#CC3028","#00529F","#403c3c","#01b699"))+
