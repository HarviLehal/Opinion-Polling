...
py_run_file("UK/party_approval/data.py")
poll <- read_csv("UK/party_approval/net_approval.csv")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  ggtitle('United Kingdom Party Net Approval Polling')
plot1
...
plot2<-ggplot(data=d1, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  geom_text(aes(label = formattable::percent(d1$value, digits = 2),y = 0),
...
plot
ggsave(plot=plot, file="UK/party_approval/plot.png",width = 20, height = 7.5, type = "cairo-png")
