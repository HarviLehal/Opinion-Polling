...
py_run_file("UK/preferred/data.py")
poll <- read_csv("UK/preferred/poll.csv")
...
plot2<-ggplot(data=d1, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#c70000","#0077b6"))+
...
plot
ggsave(plot=plot, file="UK/preferred/plot.png",width = 20, height = 7.5, type = "cairo-png")
