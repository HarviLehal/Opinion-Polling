...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_text(aes((election),h,label = "Majority (326 Seats)",hjust=1 ,vjust = -1),colour="#56595c")+
...
plot2<-ggplot(data=d3, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#f27999","#E4003B","#77c0ed","#0087DC","#fcd38b","#FAA61A",
                               "#fcf7c5","#FDF38E","#e5a694","#d46a4c","#70d2e5","#10b4d4",
                               "#9dc7af","#528D6B","#669d98","#005b54","#7ccb7e","#25a928",
                               "#fbe080","#f8cc2c","#7972a2","#201464","#c6c4fd","#a09cfc",
                               "#c2c2c2","#999999","#88a2a2","#386464"
                               ))+
  geom_text(aes(label = ifelse(d3$Date != min(d3$Date), d3$value, ""),y = 0),
            hjust=0, color="#000000",position = position_dodge(0.9), size=3.5)+
  geom_text(aes(label = ifelse(d3$Date == min(d3$Date),paste("(",d2$value,")"),""),
                y = 0),hjust=0, color="#404040", position = position_dodge(0.8), size=3.5)+
  theme_minimal()+
