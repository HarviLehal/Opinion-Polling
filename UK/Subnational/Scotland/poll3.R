...
d <- d %>%
  group_by(variable) %>%
  arrange(Date) %>%
  mutate(Moving_Average = rollapply(value, width=8, FUN=function(x) mean(x, na.rm=TRUE), by=1, by.column=TRUE, partial=TRUE, fill=NA, align="left"))

plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  geom_point(size=0.5, data=d[d$Date!=old,]) +
  scale_color_manual(values = c("#decb10","#0087DC","#E4003B","#FAA61A","#528D6B","#005eb8","#12B6CF"))+
  theme_minimal()+
...
plot3<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#ede480","#decb10","#77c0ed","#0087DC",
                               "#f27999","#E4003B","#fcd38b","#FAA61A","#9dc7af","#528D6B","#669ed4","#005eb8","#80dae8","#12B6CF"))+
  ...
  geom_text(aes(label = ifelse(d4$Date == min(d4$Date),ifelse(is.na(d4$value)==FALSE,
                               paste("(",formattable::percent(d4$value, digits = 2),")"),"Did Not Stand"),""),
...
plot1a<-ggarrange(plot1, plot3,ncol = 2, nrow = 1,widths=c(2,0.5))
plot2a<-ggarrange(plot2, plot3,ncol = 2, nrow = 1,widths=c(2,0.5))
ggsave(plot=plot1a, file="UK/Subnational/Scotland/plot_Holyrood_Constituency_ma.png",width = 15, height = 7.5, type = "cairo-png")
ggsave(plot=plot2a, file="UK/Subnational/Scotland/plot_Holyrood_Constituency.png",width = 15, height = 7.5, type = "cairo-png")
