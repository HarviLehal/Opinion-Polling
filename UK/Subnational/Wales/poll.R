...
plot2<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_vline(xintercept=Davies, linetype="dashed", color = "#0087DC", alpha=0.5, size=1)+
  geom_vline(xintercept=Rhun, linetype="dashed", color = "#1e5b53", alpha=0.5, size=1)+
  geom_vline(xintercept=Gething, linetype="dashed", color = "#E4003B", alpha=0.5, size=1)+
  geom_text(aes(Davies-5,f,label = "RT Davies", vjust = -1, hjust=0, angle=-90),colour="#0087DC")+
  geom_text(aes(Rhun-5,f,label = "ap Iorwerth", vjust = -1, hjust=0, angle=-90),colour="#1e5b53")+
  geom_text(aes(Gething-5,f,label = "Gething", vjust = -1, hjust=0, angle=-90),colour="#E4003B")+
...
plot4<-ggplot(data=d3, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#f27999","#E4003B","#77c0ed","#0087DC","#74b0a9","#1e5b53",
                               "#fcd38b","#FAA61A","#80dae8","#12B6CF","#9dc7af","#528D6B"))+
