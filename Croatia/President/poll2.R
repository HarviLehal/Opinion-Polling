...
library(data.table)
library(hrbrthemes)
py_run_file("Croatia/President/data2.py")
poll <- read_csv("Croatia/President/poll2.csv")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)

firstround<-as.Date("29 12 2024", "%d %m %Y")
election<-as.Date("12 01 2025", "%d %m %Y")
old <-min(d$Date)
# MAIN GRAPH
...
plot<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_color_manual(values = c("#ed1c24","#005baa"))+
  ...
  geom_vline(xintercept=election, linetype="solid", color = "#000000", alpha=0.75, size=0.75)+
  geom_vline(xintercept=firstround, linetype="dashed", color = "#56595c", alpha=0.5, size=0.75)+
  ...
  ggtitle('Opinion Polling for the 2024-25 Croatian Presidential election (Second Round)')
plot <- plot + guides(color = guide_legend(override.aes = list(fill = c("white", "white"), shape = c(NA, NA))))


plot

ggsave(plot=plot, file="Croatia/President/plot2.png",width = 15, height = 7.5, type="cairo-png")
