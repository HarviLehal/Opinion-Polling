...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_color_manual(values = c("#255AAA","#DF262D","#203d71",
                                "#CBE264","#DC5A2D","#bbbdbe"))+
  geom_smooth(method="loess",fullrange=FALSE,se=FALSE,span=0.9,linewidth=0.75, data=d[d$Date!=old,])+
  theme_minimal()+
...
plot2<-ggplot(data=d3, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#92add5","#255AAA","#ef9396","#DF262D","#798baa","#203d71",
                               "#e5f1b2","#CBE264","#eead96","#DC5A2D","#dddedf","#bbbdbe"))+
