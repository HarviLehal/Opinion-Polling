...
py_run_file("Croatia/data2.py")
poll <- read_csv("Croatia/poll2.csv")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)
election<-as.Date("17 04 2028", "%d %m %Y")
old<-min(d$Date)
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  ggtitle('Opinion Polling for the Next Croatian Parliamentary Election')
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#669dcc","#005baa","#f4777c","#ed1c24","#c9c9c9","#a5a5a5",
                               "#8f8f8f","#444444","#798baa","#203d71","#dbe790","#c3d746",
  ...
  geom_text(aes(label = ifelse(d4$Date == min(d4$Date),
                               ifelse(is.na(d4$value)==TRUE,
                                      ifelse(d4$variable=='DOMiNO'|d4$variable=='PiP',paste("(Coalition with DP)"),
                                             ifelse(d4$variable=='Centar'|d4$variable=='HSS',paste("(Coalition with SDP)"),
                                                    ifelse(d4$variable=='HSU'|d4$variable=='HNS',paste("(Coalition with HDZ)"),
                                                           ifelse(d4$variable=='HS',paste("(Coalition with Most)"),
                                                                  paste("(",formattable::percent(d4$value,digits=2),")"))))),paste("(",formattable::percent(d4$value,digits=2),")")),""),
                y = 0),hjust=-0.00, color="#000000", position = position_dodge(0.8), size=3.5, fontface="bold.italic")+
  theme_minimal()+
...
ggsave(plot=plot, file="Croatia/plot2.png",width = 15, height = 7.5, type="cairo-png")
