...
poll <- read_csv("Dutch/poll_govt.csv")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_smooth(method="loess",fullrange=FALSE,se=FALSE,span=0.4,linewidth=0.75, data=d)+
  theme_minimal()+
  ...
  scale_x_date(date_breaks = "1 month", date_labels =  "%m %Y",limits = c(old,max),guide = guide_axis(angle = -90))+
  ggtitle('Government-Opposition Seat Projections for the Next Dutch General Election')
...
ggsave(plot=plotA, file="Dutch/plot_govt.png",width = 20, height = 10, type="cairo-png")

ggsave(plot=plotB, file="Dutch/plot_govt_ma.png",width = 20, height = 10, type="cairo-png")
