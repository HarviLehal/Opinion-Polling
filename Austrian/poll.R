...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable))+
  ...
  geom_vline(xintercept=old, linetype="solid", color = "#000000", alpha=0.5, size=0.25)+
  geom_hline(aes(yintercept=0), alpha=0)+
  geom_point(size=1, data=d[d$Date!=old&d$Date!=election,],alpha=0.5)+
  scale_color_manual(values = c("#0056a2","#72c6d3","#ce000c",
                                "#e84188","#88b626","#ab0000","#aaaaaa"))+
  ...
  ggtitle('Opinion Polling for the Next Austrian Legislative election')
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#669ac7","#0056a2",
                               "#aadde5","#72c6d3",
                               "#e2666d","#ce000c",
                               "#f18db8","#e84188",
                               "#b8d37d","#88b626",
  ...
  theme(legend.position = "none",
        ...
        panel.background = element_rect(fill="#FFFFFF",color="#FFFFFF"),
        plot.background = element_rect(fill = "#FFFFFF",color="#FFFFFF"))+
  ggtitle('7 Day Average <br> *(2024 Results)*')+
