...
poll <- read_csv("Polish/poll2.csv")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_smooth(method="loess",fullrange=FALSE,se=FALSE,span=0.3,linewidth=0.75, data=data[data$Date!=old,])+
  ...
  ggtitle('Opinion Polling for the Next Polish Parliamentary Election by Coalition ')
plot1
...
plot2<-ggplot(data=d3, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#9581bc","#4f2d8f","#79d1cc","#20B2AA","#7d87ae","#263778"))+
  ...
  scale_x_discrete(limits = d3$variable[order(d2$value,na.last = FALSE)],labels = label_wrap(9))+
  coord_flip()
...
ggsave(plot=plot, file="Polish/plot2.png",width = 15, height = 7.5, type = "cairo-png")
