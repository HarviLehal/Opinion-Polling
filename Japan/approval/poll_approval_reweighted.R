...
library(data.table)
library(hrbrthemes)
poll <- read_csv("Japan/approval/poll_approval2.csv")
...
plot1<-ggplot(data=d[d$Date!=old,],aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  ggtitle('Kishida Cabinet Approval (Excluding Undecided)')
plot1 
...
poll <- read_csv("Japan/approval/poll_approval_net2.csv")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)
...
plot1<-ggplot(data=d[d$Date!=old,],aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_vline(xintercept=election, linetype="solid", color = "#56595c", alpha=0.5, size=0.75)+
  ggtitle('Kishida Cabinet Net Approval (Excluding Undecided)')
plot1 
