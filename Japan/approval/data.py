...
df = pd.read_csv('Japan/old/approval/poll_approval.csv')
D = pd.concat([df,D]).reset_index(drop=True)
...
df = pd.read_csv('Japan/old/approval/poll_approval2.csv')
D = pd.concat([df,D]).reset_index(drop=True)
D.Date=D.Date.astype(str).apply(lambda x: dateparser.parse(x, settings={'PREFER_DAY_OF_MONTH': 'first'}))
...
df = pd.read_csv('Japan/old/approval/poll_approval_net2.csv')
D = pd.concat([df,D]).reset_index(drop=True)
D.Date=D.Date.astype(str).apply(lambda x: dateparser.parse(x, settings={'PREFER_DAY_OF_MONTH': 'first'}))
...
D['Net Approval']=D['Approve']-D['Disapprove']
D = D.drop(["Approve","Disapprove"], axis=1)
df = pd.read_csv('Japan/old/approval/poll_approval_net.csv')
D = pd.concat([df,D]).reset_index(drop=True)
D.Date=D.Date.astype(str).apply(lambda x: dateparser.parse(x, settings={'PREFER_DAY_OF_MONTH': 'first'}))
