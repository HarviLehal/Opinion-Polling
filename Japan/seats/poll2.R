...
plot1<-ggplot(data=d[d$Date!=old,],aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_hline(aes(yintercept=76), alpha=1)+
...
plot2<-ggplot(d3, aes(fill=interaction(Date,variable), y=value, x=Date)) + 
  ...
  geom_text(aes(label = ifelse(d3$Date==max(d3$Date),ifelse(d3$variable=="Gov",paste("Average Government:",d3$value),paste("Average Opposition:",d3$value)),
