...
py_run_file("Australia/State/New South Wales/data.py")
poll <- read_csv("Australia/State/New South Wales/poll.csv")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(sub("%","",d$value))/100
d$value<-formattable::percent(d$value)

election<-as.Date("30 01 2027", "%d %m %Y")
old <-min(d$Date)
# MAIN GRAPH
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  ggtitle('Opinion Polling for the 2027 New South Wales State Election')
...
ggsave(plot=plot, file="Australia/State/New South Wales/plot.png",width = 15, height = 7.5, type="cairo-png")



# COALITION

poll <- read_csv("Australia/State/New South Wales/poll2.csv")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(sub("%","",d$value))/100
...
plot1a<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_point(data=d[d$Date==old,],size=5.25, shape=5, alpha=0.5)+
  scale_x_date(date_breaks = "2 month", date_labels =  "%b %Y",limits = c(min(d$Date),election),guide = guide_axis(angle = -90))+
  ggtitle('Two Party Preference Polling for the 2027 New South Wales State Election')
...
plot2
ggsave(plot=plot2, file="Australia/State/New South Wales/plot2.png",width = 15, height = 7.5, type="cairo-png")
