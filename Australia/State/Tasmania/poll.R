...
election<-as.Date("23 03 2024", "%d %m %Y")
old <-min(d$Date)
# MAIN GRAPH
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_smooth(method="loess",fullrange=TRUE,se=FALSE,span=0.5,linewidth=0.75, data=new[new$Date!=old&new$Date!=election,])+
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#1a27a3","#B5CEFF","#2031CC",
                               "#a82e2d","#F7B7B7","#D23A38",
                               "#2e843f","#9EED8E","#3AA54F",
                               "#c6a30d","#fadc7d","#F8CC10",
                               "#82888f","#d0d4d9","#a2aab3"))+
