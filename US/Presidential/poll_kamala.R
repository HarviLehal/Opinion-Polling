...
library(data.table)
library(hrbrthemes)
py_run_file("US/Presidential/data2.py")
poll <- read_csv("US/Presidential/poll2_kamala.csv")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_x_date(date_breaks = "2 month", date_labels =  "%b %Y",limits = c(old,election),guide = guide_axis(angle = -45))+
  ggtitle('2024 US Presidential Polling*')+
  labs(caption = "*Kamala Harris is the Democratic Nominee")
plot3
plot1
...
ggsave(plot=plot, file="US/Presidential/plot_decided_kamala.png",width = 15, height = 7.5, type="cairo-png")

plot<-ggarrange(plot3, plot2,ncol = 2, nrow = 1,widths=c(2,0.6))
plot

ggsave(plot=plot, file="US/Presidential/plot_decided_ma_kamala.png",width = 15, height = 7.5, type="cairo-png")
