...
library(data.table)
library(hrbrthemes)
py_run_file("New Zealand/data.py")
poll <- read_csv("New Zealand/poll.csv")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  ggtitle('Polling for the Next New Zealand General Election')
plot1
...
ggsave(plot=plot, file="New Zealand/plot.png",width = 15, height = 7.5, type="cairo-png")


# COALITION

poll <- read_csv("New Zealand/poll2.csv")
d <- reshape2::melt(poll, id.vars="Date")
d$value<-as.numeric(d$value)/100
d$value<-formattable::percent(d$value)
...
plot1a<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_point(data=d[d$Date==old|d$Date==election,],size=5.25, shape=5, alpha=0.5)+
  scale_x_date(date_breaks = "2 month", date_labels =  "%b %Y",limits = c(old,election),guide = guide_axis(angle = -90))+
  ggtitle('Polling for the Next New Zealand General Election (Grouped by Potential Coalitions)')
plot1a
...
ggsave(plot=plota, file="New Zealand/plot2.png",width = 15, height = 7.5, type="cairo-png")
