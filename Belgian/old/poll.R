...
plot2<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  geom_vline(xintercept=election,
             linetype="solid", color = "#56595c", alpha=0.5, size=0.75)+
  geom_point(data=d[d$Date==old|d$Date==election,],size=5, shape=18, alpha=0.8)+
  geom_point(data=d[d$Date==old|d$Date==election,],size=5.25, shape=5, alpha=0.8)+
...
plot2a<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  geom_bar(stat="identity",width=0.9, position=position_dodge())+
  scale_fill_manual(values = c("#ff8585","#ff6666","#ff0000","#85a7d7","#6691cd","#0047ab",
                               "#b0d8a0","#9cce88","#5aad39","#c78585","#b96666","#8b0000",
                               "#a3f0e9","#8cece3","#40e0d0","#ef85c2","#eb66b3","#dd0081"))+
