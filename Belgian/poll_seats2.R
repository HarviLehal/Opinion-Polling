...
poll <- read_csv("Belgian/seats2.csv")
...
plot1<-ggplot(data=d,aes(x=Date,y=value, colour=variable, group=variable)) +
  ...
  scale_color_manual(values = c("#7777FF","#FF7777"))+
...
plot2<-ggplot(data=d4, aes(x=variable, y=value,fill=interaction(Date,variable), group=Date )) +
  ...
  scale_fill_manual(values = c("#adadff","#7777FF","#ffadad","#FF7777"))+
  ...
  scale_x_discrete(limits = d4$variable[order(d3$value,na.last=FALSE)],labels = label_wrap(7))+
  coord_flip()
plot<-ggarrange(plot1, plot2,ncol = 2, nrow = 1,widths=c(2,0.5))

plot
ggsave(plot=plot, file="Belgian/plot_seats2.png",width = 15, height = 7.5, type="cairo-png")
